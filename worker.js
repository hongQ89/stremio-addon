...
const manifest = {
    id: "org.stremio.freepornmovies", // MATCHED with your local manifest
    version: "10.0.0",
    name: "Free Porn Movies (V10)",
    description: "Stable Cloudflare Worker - Identical to Local",
...
async function handleMeta(args) {
    const id = args.id.replace('fpm:', '').replace('fpm_', ''); // Handle both prefix types just in case
...
export default {
    async fetch(request) {
        ...
        const id = rawId ? decodeURIComponent(rawId) : null; // FIXED: Decode the ID
