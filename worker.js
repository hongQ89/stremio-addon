...
const manifest = {
    ...
    version: "1.10.0",
    name: "Free Porn Movies",
    description: "Clean Scraper for FreePornMovies.net",
    resources: ["catalog", "stream", "meta"],
    types: ["movie"],
    idPrefixes: ["fpm_"],
    catalogs: [
        { id: "fpm_latest", name: "Latest", type: "movie", extra: [{ name: "genre", options: ["Anytime", "Last 3 days", "This week", "This month", "Last 3 months", "Last 6 months"] }, { name: "skip" }] },
        { id: "fpm_trending", name: "Trending", type: "movie", extra: [{ name: "genre", options: ["Today", "This Week", "This Month", "All Time"] }, { name: "skip" }] },
        { id: "fpm_top_rated", name: "Top Rated", type: "movie", extra: [{ name: "genre", options: ["This Week", "All Time", "This Month", "Today"] }, { name: "skip" }] },
        { id: "fpm_most_viewed", name: "Most Popular", type: "movie", extra: [{ name: "genre", options: ["All Time", "This Month", "This Week", "Today"] }, { name: "skip" }] }
    ]
};

// V22 FIX: Balikin katalog studio individual (Inline)
...
async function handleStream(args) {
    ...
    try {
        ...
        // V22 FIX: Hantam semua models tanpa filter studio (sesuai request)
        ...
        const streamPromises = qualities.map(async (q) => {
            ...
            const r = await fetch(clean, { method: 'GET', redirect: 'manual', headers: { 'User-Agent': UA_SHORT, 'Referer': videoUrl } });
            const finalUrl = r.headers.get('location') || clean;
            const richTitle = [title, '', `📺 Res: ${q.res} • MP4`, `⏱️ Dur: ${duration}`, `🎬 Studio: ${studio}`, `👥 Models: ${models}`, `🏷️ Tags: ${tags}`, '', '✅ Verified Stream │ ✍️ @Pongky.Ir'].join('\n');
            return {
                name: `FPM • ${q.label}\n${q.res}`,
                title: richTitle,
                url: finalUrl,
                behaviorHints: { proxyHeaders: { "request": { "User-Agent": UA_SHORT, "Referer": videoUrl } } }
            };
        });
...
export default {
    async fetch(request) {
        ...
        if (path === '/manifest.json' || path === '/') return new Response(JSON.stringify(manifest), { headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' } });
