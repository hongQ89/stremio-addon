...
const manifest = {
    ...
    version: "9.0.0",
    name: "FPM Pro (Fixed V9)",
    description: "Serverless FPM - ID Prefixes Fixed",
    ...
    try {
        ...
        $('.list-videos .item').each((j, el) => {
            ...
            if (link && link.includes('/videos/')) {
                ...
                if (videoIdMatch) {
                    metas.push({
                        id: `fpm:${videoIdMatch[1]}`, // FIXED: use colon
                        type: 'movie',
                        name: title,
                        poster: thumb,
                        background: thumb,
...
async function handleMeta(args) {
    const id = args.id.replace('fpm:', ''); // FIXED: strip colon
...
async function handleStream(args) {
    ...
    const videoUrl = `${BASE_URL}/videos/${id}/`;
    try {
        const response = await fetch(videoUrl, { headers: { 'User-Agent': UA, 'Referer': BASE_URL } });
        const html = await response.text();
        const $ = cheerio.load(html);
